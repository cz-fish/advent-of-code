...
def numeric_code(code, intermediate_robots):
    ...
    for key in code:
        ...
        dx = tx - x
        dy = ty - y
        out.append(one_move(intermediate_robots, x, y, dx, dy, (0, 3)))
        x = tx
        y = ty
    return ''.join(out)
...
def one_move(level, sx, sy, dx, dy, avoid):
    if (level, sx, sy, dx, dy) in g_cache:
        return g_cache[(level, sx, sy, dx, dy)]
    options = []
    ...
    if level == 0:
        ...
        for opt in options:
            ...
            if shortest is None or len(expanded) < len(shortest):
                shortest = expanded
    g_cache[(level, sx, sy, dx, dy)] = shortest
    return shortest
...
def next_robot_step(level, instr):
    ...
    for key in instr:
        ...
        dx = tx - x
        dy = ty - y
        out.append(one_move(level, x, y, dx, dy, (0, 0)))
        x = tx
        y = ty
    line = ''.join(out)
    #print(level, line)
